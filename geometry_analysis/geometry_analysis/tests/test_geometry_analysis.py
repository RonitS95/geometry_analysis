...
def test_calculate_distance():
    ...
    assert expected_dist == calculated_distance
...
def test_molecule_set_coordinates(water_molecule):
    """Tests that bonds are rebuilt when we reset coords"""
...
@pytest.mark.parametrize("p1, p2, p3, expected_angle",[
    ...
    (np.array([1,0,0]), np.array([0,0,0]), np.array([1,1,0]), 45),
])

def test_calculate_angle_param(p1, p2, p3, expected_angle):
